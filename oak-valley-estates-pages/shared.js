...
const BUDGET_URL='https://docs.google.com/spreadsheets/d/e/2PACX-1vQew4OQL4AsLo127rU7ZX8KC6Ur4BklOWahgzWE99HsNQzrEq2Re0cqFpDIofBkW39nXzTvx0cX5Los/pub?gid=128071023&single=true&output=csv';
...
function parseBudgetTab(rows){
  ...
  if(rows[3]){
    // Prefer the "Contract" total from the Build Gross row (original contract),
    // not the "Budget" column which may already include change orders.
    totalBudget=parseAmt(rows[3][1])||parseAmt(rows[3][6])||parseAmt(rows[3][2])||0;
...
async function loadProjectData(){
  ...
  const baseBudgetFromHeader=Number(headerInfo&&headerInfo.budget)||0;
  const basePaidFromGross=Number(buildGrossTotals&&buildGrossTotals.paid)||0;
  if(baseBudgetFromHeader>0){
    totalBudget=baseBudgetFromHeader;
  }else{
    for(const p of phases){
      if(p.name==='options'||p.name==='contingency')continue;
  ...
  if(basePaidFromGross>0){
    totalPaid=basePaidFromGross;
  }else{
    for(const p of phases){
      if(p.name==='options'||p.name==='contingency')continue;
